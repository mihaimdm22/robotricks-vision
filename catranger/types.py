...
class Detection:
    ...
    cls_id: int
    cls_name: str = "cat"
    track_id: int | None = None  # set by the tracker; None if detection-only
...
class CatObservation:
    ...
    detection: Detection
    distance: DistanceResult | None = None
    bearing_deg: float = 0.0  # horizontal angle from optical axis (+ = right)
    speed_mps: float | None = None  # approach/recede speed if track history allows
