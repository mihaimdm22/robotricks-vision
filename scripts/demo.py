...
def parse_args():
    ...
    p.add_argument("--source", required=True, help="image dir | image | video | rtsp url | webcam index")
    ...
    p.add_argument("--no-depth", action="store_true", help="disable the depth network (geometry only)")
    ...
    p.add_argument("--hw-port", default=None,
                   help="Arduino link: USB serial (/dev/ttyACM0) or Bluetooth-SPP port (/dev/cu.HC-05...)")
    p.add_argument("--ble", default=None, help="BLE address of an HM-10-style module (enables robot output over BLE)")
    p.add_argument("--connection", default="auto", choices=["auto", "usb", "bt", "ble", "dummy"],
                   help="robot transport. 'bt'=HC-05 SPP serial (9600 baud), 'ble'=HM-10")
    p.add_argument("--baud", type=int, default=115200, help="serial baud (USB 115200; HC-05 SPP usually 9600)")
...
def main():
    ...
    if link_target:
        ...
        print(f"[catranger] robot link: connection={connection} target={link_target} "
    ...
    print(f"[catranger] camera={app.camera.name} approach={approach} "
    ...
    for idx, frame in frame_source(args.source, stride=args.stride, max_frames=args.max_frames):
        ...
        annotated = draw(result_frame := ranger.last_undistorted if hasattr(ranger, "last_undistorted") else frame,
                         result, command)

        # report line
        tgt = result.target
        if tgt and tgt.distance:
            print(f"frame {idx:4d} | {len(result.observations)} cat(s) | "
                  ...
                  + (f" | cmd={command.state} rot={command.rotation:+.2f} v={command.v_fwd:+.2f}" if command else ""))
        if args.print_json:
            print(json.dumps({
                "frame": idx,
                "observations": [
                    {"id": o.track_id, "dist_m": round(o.distance.meters, 3) if o.distance else None,
                     "bearing_deg": round(o.bearing_deg, 1)} for o in result.observations
                ],
                "command": command.as_dict() if command else None,
            }))

        if save_dir is not None:
            if str(args.source).isdigit() or "://" in str(args.source) or Path(str(args.source)).suffix.lower() in {".mp4", ".mov", ".avi", ".mkv"}:
                if writer is None:
                    h, w = annotated.shape[:2]
                    writer = cv2.VideoWriter(str(save_dir) + ".mp4",
